...
2. Выбрать любую понравившуюся модель/задачу обработки изображения нейросетью.
   ...
   В случае если выбрана модель YOLO из репа по ссылке выше для распознавания объектов, то необходимо модифицировать скрипт для отображения bounding box'ов объектов с меткой класса и метрики: [пример задания для YOLO v5](sub_task_pytorch.md).
3. Запустить контейнер командой:
   ```bash
   docker compose -f <имя_конфига.yaml> up
   ```
4. Запустить скрипт с реализованным алгоритмом в контейнере в примонтированной внутри контейнера папке. 
