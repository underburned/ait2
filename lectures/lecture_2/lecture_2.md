...
Для проброса GPU внутрь контейнера необходимо установить *NVIDIA Container Toolkit* согласно инструкции [Installing the NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html). Сначала необходимо добавить реп в списки источников `apt`:
...
Установка необходимых пакетов (версия `1.17.8-1` актуальная на 10.10.2025):
...
> Процедуры установки CUDA/драйвера/NVIDIA Container Toolkit меняются как от релиза новых версий дистрибутивов Linux, так и от модификаций "схемы распространения" самой NVIDIA (изменение URL репов NVIDIA, ключей цифровой подписи репов (GPG), названий пакетов и т.п.). Так что имеет смысл перед установкой на чистую ОС ознакомиться с актуальной версией документации.
